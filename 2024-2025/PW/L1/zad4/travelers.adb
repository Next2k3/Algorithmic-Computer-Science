...
procedure Travelers is
  ...
  type Board_Type is array (0..Board_Width-1, 0..Board_Height-1) of Cell_Type;
  ...
  task body Traveler_Task_Type is
    ...
    Step: Integer := 0; 
    ToLower: constant Integer := Character'Pos('a') - Character'Pos('A'); 
    N: Integer;
  ...
  begin
    accept Init(Id: Integer; Seed: Integer; Symbol: Character) do
      ...
      N := Integer( 2.0 * Random(G) ) + ( Id mod 2 ) * 2;
    end Init;
    ...
    select
      ...
      Store_Trace;
    or
      delay Nr_Of_Travelers * Max_Delay;
      Step := Nr_of_Steps;
    end select;

    while Step < Nr_of_Steps loop
      delay Min_Delay + (Max_Delay - Min_Delay) * Duration(Random(G));
      Traveler.Prev_Position.X := Traveler.Position.X;
      Traveler.Prev_Position.Y := Traveler.Position.Y;
      Make_Step;
      select
        Board(Traveler.Position.X, Traveler.Position.Y).Enter; 
        Time_Stamp := To_Duration(Clock - Start_Time);
        Store_Trace;
        Board(Traveler.Prev_Position.X, Traveler.Prev_Position.Y).Leave; 
      or
        delay Nr_Of_Travelers * Max_Delay;
        Step := Nr_of_Steps;
        Traveler.Symbol := Character'Val(Character'Pos(Traveler.Symbol) + ToLower); 
        Traveler.Position.X := Traveler.Prev_Position.X;
        Traveler.Position.Y := Traveler.Prev_Position.Y;
        Time_Stamp := To_Duration(Clock - Start_Time);
        Store_Trace;
      end select;
      Step := Step + 1;
