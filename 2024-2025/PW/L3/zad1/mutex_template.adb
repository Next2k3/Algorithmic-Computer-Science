...
-- Types, procedures and functions
  ...
  type Trace_Array_type is  array(0 .. Max_Steps * 4) of Trace_Type;
...
-- add Ticket
  protected Ticket_Maximum is
    ...
    protected body Ticket_Maximum is
...
--
  ...
  begin
    ...
    Put_Line("MAX_TICKET=" & Trim(Integer'Image(Ticket_Maximum.Get_Max), Left) & ";");
...
-- add
    type Process_ID is range 0 .. Nr_Of_Processes - 1;

    Choosing : array(Process_ID) of Boolean := (others => False);
    Number   : array(Process_ID) of Integer := (others => 0);
...
-- add 
    for Step in 0 .. Nr_of_Steps - 1  loop
      ...
      Choosing(Process_ID(Process.Id)) := True;
      Number(Process_ID(Process.Id)) := Max + 1;

      if Number(Process_ID(Process.Id)) > Max_Ticket_Proc then
        Max_Ticket_Proc := Number(Process_ID(Process.Id));
      end if;

      Choosing(Process_ID(Process.Id)) := False;

      for J in Process_ID loop
        if J /= (Process_ID(Process.Id)) then
          while Choosing(J) loop
            null;
          end loop;

          while Number(J) /= 0 and then
              (Number(J) < Number(Process_ID(Process.Id)) or
              (Number(J) = Number(Process_ID(Process.Id)) and then J < Process_ID(Process.Id))) loop
            null;
          end loop;
        end if;
      end loop;
      ...
      Number(Process_ID(Process.Id)) := 0;
      Change_State( LOCAL_SECTION );
    end loop;

    
    Ticket_Maximum.Update_Max(Max_Ticket_Proc);
