...
func entryProtocolPeterson(id int, flag *[2]bool, turn *int) {
	other := 1 - id
	flag[id] = true
	*turn = other
...
func exitProtocolPeterson(id int, flag *[2]bool, turn *int) {
	flag[id] = false
}
...
func process(id int, seed int64, symbol rune, startTime time.Time, reports chan<- TraceReport, wg *sync.WaitGroup) {
	...
	for step := 0; step < nrOfSteps; step++ {
		...
		// ENTRY_PROTOCOL
		entryProtocolPeterson(id, &flag, &turn)
		...
		// EXIT_PROTOCOL
		exitProtocolPeterson(id, &flag, &turn)
		time.Sleep(MinDelay + time.Duration(r.Int63n(int64(MaxDelay-MinDelay))))
