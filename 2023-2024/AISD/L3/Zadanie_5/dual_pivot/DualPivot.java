...
public class DualPivot {
    ...
    static int partition(int[] arr, int low, int high, int[] lp) {
        ...
        int s = 0; // counter
        while (k <= g) {
            // if the number of big elements (greater than q) is greater than the number of small elements (less than p)
            // compare with q first, then if necessary, with p
            if (l > s) {
            ...
            // if the number of big elements (greater than q) is less than or equal to the number of small elements (less than p)
            // compare with p first, then if necessary, with q
            else {
                if (compareKeys(arr[k], p)) {
                    swapKeys(arr, k, i);
                    s++;
